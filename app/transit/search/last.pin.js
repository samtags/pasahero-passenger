...
export default function Pin() {
  ...
  let activeShortAddress = undefined;

  if (active) {
    activeShortAddress = `${active?.address_components?.[0]?.long_name} ${active?.address_components?.[1]?.long_name}` // prettier-ignore
  }

  const isDisabled = !isDragging || isLoading;
  ...
  function handleConfirm() {
    handleSetTransitLast({
      ...
      longAddress: activeShortAddress,
    });
